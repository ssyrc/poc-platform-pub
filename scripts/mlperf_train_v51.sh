...
LOG_ROOT="${MLPERF_LOG_ROOT:-${POC_PLATFORM_ROOT:-/opt/poc-platform}}/mlperf_logs_train_v51"
CONFIG_PATH=""
DRY_RUN="false"
...
ensure_image() {
  ...
  # Name every path that was tried. On a multi-node run this failure surfaces
  # on one host while the others are fine, and "missing" alone does not say
  # whether the tar was absent, unreadable, or simply somewhere else on
...
if [[ "$EXIT_CODE" -eq 0 ]]; then
  ...
  echo "[PHASE] done"
  emit_summary "failed" "$EXIT_CODE" "training v5.1 failed; inspect container.log, run.log and command.txt"
  exit "$EXIT_CODE"
fi
REMOTE_RUN
